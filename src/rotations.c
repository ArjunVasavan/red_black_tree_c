...
void right_rotate(node **root, node *node_y) {
    ...
    if ( node_x->right_pointer != NULL ) {
        node_x->left_pointer->parent_pointer = node_y;
    }

    if ( node_y->parent_pointer == NULL ) {

        *root = node_x;
        node_x->parent_pointer = NULL;

    } else {

        node_x->parent_pointer = node_y->parent_pointer;

        if ( node_y == node_y->parent_pointer->left_pointer ) {
            node_y->parent_pointer->left_pointer = node_x;
        } else {
            node_y->parent_pointer->right_pointer = node_x;
        }
    }
    node_x->right_pointer = node_y;
    node_y->parent_pointer = node_x;
}
